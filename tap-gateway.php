<?php
/*
 * Plugin Name: Tap Payments Gateway
 * Plugin URI: https://www.tap.company/
 * Description: Take credit card payments on your store using tap payments.
 * Author: Fahhad Alsubaie
 * Author URI: https://fahhad.io
 * Version: 2.1.1
 * Requires at least: 5.2
 * Requires PHP:      7.2
 * @copyright         2023 Tap Payments
...
{
	$gateways[] = 'WC_Tap_Payments_Gateway';
	return $gateways;
}
...
function tap_init_gateway_class()
{
	...
	class WC_Tap_Payments_Gateway extends WC_Payment_Gateway
	{
		...
		public function __construct()
		{
			...
			$this->method_description = 'Tap Payments Gateway - Take credit card payments on your store.'; // will be displayed on  the options page
			$this->supports = array(
				'products',
				'refunds'
			);
			...
			// Load the settings.
			$this->init_settings();
			$this->icon = tap_imgdir . 'logo.jpeg';
		...
		public function tap_thank_you_page($order_id)
		{
			...
			if (!empty($_GET['tap_id']) && $this->payment_mode == 'charge') {
				...
				if ($response->status !== 'CAPTURED' || $response->status == 'FAILED'  || $response->status == 'CANCELLED' && $response->post->status == 'ERROR') {
					$order->update_status('pending');
			...
			if ($response->status !== 'CAPTURED' || $response->status == 'FAILED'  || $response->status == 'CANCELLED' && $response->post->status == 'SUCCESS') {
				$items = $order->get_items();
				foreach ($items as $item) {
					$product_name = $item->get_name();
					$product_quantity = $item->get_quantity();
		...
		public function tap_checkout_receipt_page($order_id)
		{
			...
			$order_des = ('Order ID:') . ($order->get_id());
			...
			echo '<input type="hidden" id="SiteUrl" value="' . $site_Url . '" />';
			...
			echo '<input type="hidden" id="billing_first_name" value="' . $order->get_billing_first_name() . '" />';
			echo '<input type="hidden" id="billing_last_name" value="' . $order->get_billing_last_name() . '" />';
			...
			foreach ($items as $key => $item) {
				...
				echo '<input type="hidden" name="items_bulk[]" data-name="' . $item['data'] . '" data-quantity="' . $item['quantity'] . '" data-sale-price="' . $unit_price . '" data-item-product-id="' . $item['product_id'] . '" data-product-total-amount="' . $product_total_amount . '" data-total-amount="' . $order_total . '" class="items_bulk">';

			}
			if ($this->ui_mode == 'redirect') {
				$trans_object["amount"]                   = $order->get_total();
				$trans_object["currency"]                 = $order->get_currency();
				$trans_object["threeDsecure"]             = true;
				$trans_object["save_card"]                = false;
				$trans_object["description"]              = "ORDER ID :" . $order->get_id();
				...
				$trans_object["reference"]["order"]        = $order->get_id();
				$trans_object["receipt"]["email"]          = false;
				$trans_object["receipt"]["sms"]            = true;
				$trans_object["customer"]["first_name"]    = $order->get_billing_first_name();
				$trans_object["customer"]["last_name"]    = $order->get_billing_last_name();
				$trans_object["customer"]["email"]        = $order->get_billing_email();
				$trans_object["customer"]["phone"]["country_code"]       = '';
				$trans_object["customer"]["phone"]["number"] = $order->get_billing_phone();
				$trans_object["source"]["id"] = 'src_all';
				$trans_object["post"]["url"] = $ServerNotificationUrl;
				$trans_object["redirect"]["url"] = $this->get_return_url($order);
		...
		public function init_form_fields()
		{
			$this->form_fields = array(
				'enabled' => array(
					...
					'label'       => 'Enable Tap Payments Gateway',
					'type'        => 'checkbox',
					'description' => '',
					'default'     => 'no'
				),
				'title' => array(
					'title'       => 'Title',
					'type'        => 'text',
					'description' => 'Title which the user sees on checkout page.',
					'default'     => 'Credit Card',
					'desc_tip'    => true,
				),
				'description' => array(
					'title'       => 'Description',
					'type'        => 'textarea',
					'description' => 'Description which the user sees on checkout page.',
					'default'     => 'Pay with your credit card via Tap payment gateway. Once you click "place order", you will be redirected to the payment page.',
				),
				'testmode' => array(
					'title'       => 'Test mode',
				...
				'save_card' => array(
					...
					'type'        => 'checkbox',
					'description' => 'Saving card data on your website leads to leaking customer details if hacked, please be aware. (recommended is no)',
					'default'     => 'no'
				)
